var builder = require("../../bin/builder"), common = require("./builder.common"), should = require("should");
...
module.exports = {
    ...
    "production build LOC": function() {
        builder(function(err, result) {
            ...
            Buffer.byteLength(result).should().be.below(43e3);
        });
    },
    "development build LOC": function() {
        builder({
            ...
            Buffer.byteLength(result).should().be.above(35e3);
        });
    },
    "default builds": function() {
        builder(function(err, result) {
            should.strictEqual(err, null);
            var io = common.execute(result).io, transports = Object.keys(io.Transport), defaults = Object.keys(builder.transports);
    ...
    "custom build": function() {
        builder([ "websocket" ], function(err, result) {
            should.strictEqual(err, null);
            var io = common.execute(result).io, transports = Object.keys(io.Transport);
    ...
    globals: function() {
        builder(function(err, result) {
            should.strictEqual(err, null);
            var io = common.execute(result), env = common.env(), allowed = [ "io", "swfobject", "WEB_SOCKET_DISABLE_AUTO_INITIALIZATION" ];
            Array.prototype.push.apply(allowed, Object.keys(env));
            Object.keys(io).forEach(function(global) {
                var index = allowed.indexOf(global);
                if (!~index) throw new Error("Global leak: " + global);
            });
        });
    }
};